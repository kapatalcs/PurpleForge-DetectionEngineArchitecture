...
class CredentialStuffingAttackRule(BaseRule):
    ...
    name = "distributed_credential_stuffing_attack"

    def __init__(
        self,
        min_unique_users: int = 9,
        min_unique_ips: int = 5,
    ...
    def match(self, event: Event, engine) -> Optional[dict]:
        ...
        events: List[Event] = [
            e
            for ip_events in engine.events_by_ip.values()
            for e in ip_events
                if (
                    e.user
        ...
        return {
            ...
            "reason": "Multiple sources attempted multiple stolen username-password combinations."
        }
